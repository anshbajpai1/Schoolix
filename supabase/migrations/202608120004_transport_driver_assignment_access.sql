-- Older driver accounts are assigned in Firestore, while newer accounts may
-- have a row in schoolix_transport_drivers. Treat both as the same driver.
create or replace function public.schoolix_transport_driver_can_access_vehicle(target_vehicle_id text)
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    ...
    where v.id = target_vehicle_id
      and (
        v.driver_id = public.schoolix_transport_driver_id()
        or exists (
          select 1
...
using (
  ...
  or public.schoolix_transport_driver_can_access_vehicle(id)
...
using (
  public.schoolix_can_access_school(school_id)
  or public.schoolix_transport_driver_can_access_vehicle(vehicle_id)
...
using (
  public.schoolix_can_access_school(school_id)
  or (exists (select 1 from public.schoolix_transport_trips t where t.id = trip_id and t.status = 'active')
      and public.schoolix_transport_driver_can_access_vehicle(vehicle_id))
  or exists (
...
begin
  ...
  where id = target_trip_id and vehicle_id = target_vehicle_id and status = 'active';
  if not found then raise exception 'Active trip not found'; end if;
  if not (public.schoolix_can_access_school(trip_row.school_id)
      or public.schoolix_transport_driver_can_access_vehicle(target_vehicle_id)) then
    raise exception 'Access denied';
  end if;
  insert into public.schoolix_vehicle_live_locations (
    vehicle_id, school_id, trip_id, latitude, longitude, speed, heading, accuracy, updated_at
  ) values (
    target_vehicle_id, trip_row.school_id, target_trip_id, target_latitude, target_longitude,
    target_speed, target_heading, target_accuracy, now()
  ) on conflict (vehicle_id) do update set
    school_id = excluded.school_id, trip_id = excluded.trip_id, latitude = excluded.latitude,
    longitude = excluded.longitude, speed = excluded.speed, heading = excluded.heading,
